...
define ('ACTIVITY_DISP',	STATE::SELECT + 4);
define ('ACTIVITY_PICK',	STATE::SELECTED + 4);
define ('HOURS_DISP',		STATE::SELECT + 5);
...
$version = "v3.1"; //goes with the downloaded timesheet file for client verification
...
case SELECTED_SPECS:
	...
	$_STATE->goback_to(SHOW_SPECS); //set goback
...
case SHEET_DISP:
	...
	case STATE::INIT:
		...
		case "AT": //activity
			...
			break;
		default:
			throw_the_bum_out(NULL,"Evicted(".$_STATE->ID."/".__LINE__."): invalid agent ".$SCION->agent,true);
		}
...
function log_put() {
	...
	$filename = "timesheet_".$orgname."_".$from."_to_".$to.".csv"; //for file_put...
	...
	foreach ($row as $name=>$value) { //headings
		if (($name=="project_id")||($name=="lastname")||($name=="firstname")
			||($name=="extension")) continue; //don't send
		//activity_id?
		$outline[] = $name;
		$fields .= ",".$name;
	}
...
function update_log(&$state, $offset) {
	...
	$sql = "UPDATE ".$_DB->prefix."b00_timelog
			SET hours=".$_POST["hours".$offset]."
			WHERE timelog_id=".$_POST["rec".$offset].";";
	$_DB->exec($sql);
...
function delete_log(&$state, $offset) {
	...
	$sql = "DELETE FROM ".$_DB->prefix."b00_timelog
			WHERE timelog_id=".$_POST["rec".$offset].";";
	$_DB->exec($sql);
...
function new_hours(&$state) {
	...
	if ($state->mode == "t") {
		...
		$record = reset($state->records); //updating in list mode
...
?>
  ...
  <tr id="add">
    <td id="BN_0" data-recid="0" title="Click to add hours for new activity">
...
function onerow(&$header, &$logs) {
	...
	for ($ndx=0; $ndx<$max; $ndx++) {
		...
		$logs[$ndx] = array(0,0); //reset back to initial values
	}
	echo "\n";
